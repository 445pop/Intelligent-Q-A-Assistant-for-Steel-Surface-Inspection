...
from yolov5.infer_nvidia.utils.downloads import gsutil_getsize
from yolov5.infer_nvidia.utils.metrics import box_iou, fitness
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-9):
    ...
    # temp = []
    # for b2x1, b2x2, b2y1, b2y2 in zip(b2_x1, b2_x2, b2_y1, b2_y2):
    #     overlap_condition = (b1_x1 < b2x2) & (b1_x2 > b2x1) & (b1_y1 < b2y2) & (b1_y2 > b2y1)
    #     print(overlap_condition)
    #     if overlap_condition:
    #         temp.append((torch.min(b1_x2, b2x2) - torch.max(b1_x1, b2x1)).clamp(0) * \
    #                     (torch.min(b1_y2, b2y2) - torch.max(b1_y1, b2y1)).clamp(0))
    #     else:
    #         temp.append(0)

    # temp = torch.tensor(temp).to('cuda:0')
    # print('TEMP:',temp)
    # are_equal = torch.equal(temp, inter)
    ...
    union = abs(w1 * h1) + abs(w2 * h2) - inter + eps
    print("inter",inter)
    print("union",union)
    iou = inter / union
    print("iou",iou)
    ...
    # for tensor in iou:
    #     print("tensor", tensor)
    #     # 使用torch.any()函数判断张量中是否存在小于0的元素
    #     if torch.any(tensor == 0).item():
    #         # 如果存在小于0的元素，则打印
    #         print("列表中存在等于0的数:", tensor)
    #         tensor = 1
    print("iou", iou)
...
def is_overlapping(rect1, rect2):
    ...
    return (b1_x1 < b2_x2) and (b1_x2 > b2_x1) and (b1_y1 < b2_y2) and (b1_y2 > b2_y1)
